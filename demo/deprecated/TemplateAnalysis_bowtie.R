...
# # cat testing.bed | awk '{if ($3 - $2 >= 18) print}' > output#this should be run when making contigs!!!
...
rootDir = file.path( "/home","simon","PHDStudies","RNA-Seq","IonProton","TestAnalyses","MiniAnalysis_bowtie1_1")#schaffrr
...
bowtieCLI_cmdResL = lapply( fastQFiles, function(fqf){
  bowtieCLI = Bowtie_CLI(inFilePath=getOutFilePath(getCLIApplication(cutAdatptCLI_cmdRes)), inFileNames=fqf, 
                         cliParams="-qSya -n 1 -p 6 --chunkmbs 1024 --best -l 18 -m 100 --strata", #--phred64-quals 
...
multiBamCov_CLI = MultiBamCov_CLI(inFilePath = "", 
                                  inFileNames = inFN,
                                  cliParams =  c("-s -f 0.05 -D"), # -D include duplicated reads 0.05 ~1bp at length 200 - 10 bp -> only good for short read data!
